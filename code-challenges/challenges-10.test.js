...
const getNames = (arr) => {
  ...
  let newArr = arr.map(val => {
    let reversedArr =val.name.split('').reverse().join('')
...
const totalSum = (input) => {
  ...
  let sum = 0;
  input.map(outerArr => {
      return outerArr.map(innerArr => sum += innerArr);
  })
  return sum;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filtered = data.filter(val => val.gender == 'male' || val.gender == 'female');
  let name = '';
